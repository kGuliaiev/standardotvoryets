          ...
          {groups?.map((g) => {
            ...
            return (
              <Link
                ...
                className="bg-card rounded-xl border border-hairline p-5 hover:border-blue-300 hover:shadow-md transition-all group"
              >
                {/* Top row */}
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="font-mono text-xs text-light">{g.code}</p>
                      <p
                        className="font-semibold text-ink group-hover:text-blue-700 transition-colors leading-tight"
                  ...
                  {myMembership && (
                    <span className="text-xs bg-pill text-mid px-2 py-0.5 rounded-full shrink-0">
                ...
                {/* Description */}
                {g.description && (
                  <p className="text-xs text-mid line-clamp-2 mb-3">{g.description}</p>
                )}

                {/* Stats */}
                <div className="flex gap-4 text-xs text-light pt-3 border-t border-hairline">
                  <span>
                    <span className="font-semibold text-mid">{g._count.members}</span> учасн.
                  </span>
                  <span>
                    <span className="font-semibold text-mid">{g._count.standards}</span> станд.
                  </span>
                </div>
              </Link>
            );
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-card rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
            <h2 className="text-lg font-semibold text-ink mb-4">Нова робоча група</h2>
            <form onSubmit={handleCreate} className="space-y-4">
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-medium text-mid mb-1">Код *</label>
                  <input
                    type="text"
                    maxLength={20}
                    placeholder="РГ-1"
                ...
                <div className="col-span-2">
                  <label className="block text-xs font-medium text-mid mb-1">Колір</label>
                  <div className="flex items-center gap-2">
                    <input
                      type="color"
                      value={form.color}
                      onChange={(e) => setForm((f) => ({ ...f, color: e.target.value }))}
                      className="w-10 h-9 rounded-lg border border-hairline cursor-pointer p-0.5"
                    />
                    <span className="text-xs text-light font-mono">{form.color}</span>
                  </div>
                </div>
              </div>
